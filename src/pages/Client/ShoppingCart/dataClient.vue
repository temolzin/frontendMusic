...
export default defineComponent({
  ...
  watch: {
    ...
    "formClient.event_type_id"() {
      if (!this.isOtherEventType) this.formClient.event_type_detail = '';
    },
            ...
            try {
              ...
              const paymentData = {
                ...
                order_details: {
                  ...
                  event_type_detail: this.isOtherEventType ? this.formClient.event_type_detail.trim() : null,
                },
                latitude: this.latitude,
                longitude: this.longitude,
                google_place_id: this.googlePlaceId,
  ...
  computed: {
    ...
    eventTypeOptions() {
      return (this.eventTypes || []).map((type) => ({ label: type.name, value: type.id }));
    },
    selectedEventTypeName() {
      const selected = (this.eventTypes || []).find((type) => type.id === this.formClient.event_type_id);
